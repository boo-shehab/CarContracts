...
const Notifications = () => {
  ...
  useEffect(() => {
    ...
    const mockData: Notification[] = [
      ...
      {
        id: "7",
        message: 'قام محمد علي <span class="text-blue-600 font-medium">بتسجيل الدخول</span> الى حسابه',
    ...
    <div className="relative font-sans">
      ...
      {open && (
        <div className="absolute top-full right-0 w-[610px] max-h-[860px] bg-white shadow-xl rounded-lg overflow-y-auto py-6 px-4 z-50">
          <div className="mb-4">
            <h2 className="text-xl font-bold text-gray-900 mb-1">الإشعارات</h2>
          ...
          {notifications.map((notif, i) => {
            const isThisWeek = i < 7; // Just a demo divider condition

            return (
              <div key={notif.id}>
                {i === 7 && (
                  <div className="my-4 border-t pt-2 text-gray-800 font-semibold text-sm">
                    هذا الاسبوع
                  </div>
                )}

                <div className="flex items-start justify-between gap-2 py-3 border-b last:border-none">
