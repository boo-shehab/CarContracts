            ...
            <div className='flex items-center gap-6'>
                ...
                <nav>
                    <ul className='hidden lg:flex gap-6'>
                        <NavLink to="/AdminDashboard" className={({ isActive }) => (`text-xl font-normal ${isActive ? 'text-primary-500 border-b' : ''}`)}>الصفحة الرئيسية</NavLink>
                    </ul>
                </nav>
            </div>
        ...
        <div className={`fixed top-0 right-0 w-3/4 sm:w-1/2 md:w-1/3 h-full bg-white shadow-lg transform transition-transform duration-300 z-50 ${isSidebarOpen? 'translate-x-0' : 'translate-x-full'}`}>
            <div className=" bg-white h-full p-4 flex flex-col gap-6">
                ...
                <nav className='flex flex-col gap-3 text-right'>
                    <NavLink to="/AdminDashboard" onClick={() => setIsSidebarOpen(false)} className={({ isActive }) => (`inline text-xl font-normal ${isActive ? 'text-primary-500 border-b' : ''}`)}>الصفحة الرئيسية</NavLink>
                </nav>
            </div>
        </div>
    </header>
